const WordRoots = [
  ...
  {
    id: 13,
    root: "命题式分块 (Proposition-based Chunking)",
    origin: "分块策略",
    meaning: "基于逻辑命题和论述单元进行文档分割",
    description: "命题式分块是一种基于逻辑结构的高级分块方法，它将文档按照逻辑命题、论证单元或观点陈述进行分割。每个分块包含一个完整的逻辑命题或论述，确保信息的逻辑完整性。这种方法特别适用于学术论文、法律文档、政策文件等具有严密逻辑结构的文本。通过保持逻辑单元的完整性，能够提高检索结果的准确性和可理解性。",
    examples: [
      {
        word: "学术论文分析",
        meaning: "研究观点提取",
        breakdown: { root: "命题式分块" },
  ...
  {
    id: 18,
    root: "GraphRAG（知识图谱增强RAG）",
    origin: "索引技术",
    meaning: "一种具体的知识图谱增强RAG方法，通过图结构组织实体关系并辅助检索与推理",
    description: "GraphRAG是知识图谱增强RAG中的具体方法/框架，而“知识图谱增强”本身是更大的方法类别。GraphRAG通常先抽取实体与关系并构建图结构，再结合图遍历/社区摘要与向量检索完成召回与推理。相比仅做向量相似度匹配，它更擅长处理关系查询、多跳问答和需要结构化证据链的场景。",
    examples: [
      {
        word: "医疗诊断系统",
        meaning: "疾病关系推理",
  ...
  },
  {
    ...
    origin: "检索方法",
    meaning: "采用token级表示与MaxSim后交互进行细粒度语义匹配",
    description: "ColBERT(Contextualized Late Interaction over BERT)是一种细粒度检索方法。它不会把整段文本压成单一向量，而是为查询和文档的每个token分别编码；在检索阶段使用Late Interaction（典型为MaxSim）聚合查询token与文档token的匹配分数。该机制通常能在效率与精度之间取得较好平衡，尤其适合需要词项级精确对齐的检索任务。",
    examples: [
      {
        word: "精确问答检索",
        meaning: "关键词精准匹配",
  ...
  },
  {
    ...
    root: "Self-RAG",
    origin: "生成优化",
    meaning: "通过反思/批判信号驱动检索-生成决策与纠错的RAG系统",
    description: "Self-RAG的关键不只是“自检”，而是在生成过程中引入反思/批判信号来显式控制流程：是否检索、检索哪些证据、是否继续生成或修正答案。系统会结合相关性、支撑性和完整性等评估信号进行决策，当证据不足或不一致时触发补充检索与重写，从而提升答案可验证性与稳定性。",
    examples: [
      {
        word: "智能问答助手",
        meaning: "答案质量自检",
  ...
  },
  {
    ...
    origin: "生成优化",
    meaning: "先评估检索质量，再按质量分流并触发补救路径的RAG框架",
    description: "Corrective RAG (CRAG)强调先做检索结果质量评估，再根据质量分层处理。高质量结果可直接用于生成；中低质量结果会触发补救路径，例如补充检索、替代查询、外部知识源验证或局部重写。其核心价值在于把“检索质量控制”前置到生成前，从而提升系统鲁棒性并降低错误证据进入答案的风险。",
    examples: [
      {
        word: "事实核查系统",
        meaning: "信息可信度评估",
